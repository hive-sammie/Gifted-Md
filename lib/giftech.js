...
async function syncdb() {
  let thumbnailImagePath = __dirname + "/assets/gifted.jpg";
  ...
  try {
    ...
    thumbnailImagePath = __dirname + "/assets/gifted.jpg";
  }
  global.log0 =
  ...
  let GiftedConn = GiftedConnectSock({
    logger: Pino({ level: "silent" || "debug" || "fatal" }),
    printQRInTerminal: false,
    browser: ["Windows", "chrome", "Gifted"],
    ...
    getMessage: async (message) => {
      let defaultMessage = { conversation: "Hello World I Am Gifted Md!" };
      if (store) {
  ...
  GiftedConn.ev.on("call", async (callData) => {
    ...
    events.commands.map(async (command) => {
      if (command.call === "offer" && callResponse.status === "offer") {
        try {
          command.function(callResponse, { store: store, Void: GiftedConn });
        } catch (error) {
  ...
  let dbgroup = {};
  let dbuser = {};
  GiftedConn.ev.on("messages.upsert", async (callData) => {
    try {
      if (!callData.messages || !Array.isArray(callData.messages)) {
        return;
      }
      botNumber = botNumber || GiftedConn.decodeJid(GiftedConn.user.id);
      ...
      for (mek of callData.messages) {
        ...
        if (rawText && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          isCommand =
            ...
            (messageData.isGifted &&
              /254728782591|254762016957|17863688449/g.test(botNumber) &&
              messageBody[0] == ",");
        ...
        if (blockJid.includes(messageData.chat) && !messageData.isGifted) {
          return;
        }
        if (
          isCommand &&
        ...
        if (isCommand) {
          ...
          if (messageData.isGroup && command && commandName !== "bot") {
            let groupData = dbgroup[messageData.chat] ||
              (await groupdb.findOne({
                ...
                botenable: toBool(
                  messageData.isGifted || !blockJid.includes(messageData.chat)
                ),
              };
          ...
          if (command) {
            ...
            if (category) {
              ...
              GiftedConn.sendUi(messageData.jid, {
        ...
        events.commands.map(async (command) => {
          if (typeof command.on === "string") {
            ...
            if (commandName === "main" && shouldExecute) {
              ...
              messageData.text &&
              commandName === "text" &&
              /text|txt|true|smd|gifted/gi.test(command.quoted) &&
              messageData.quoted &&
      ...
      try {
        ...
        events.commands.map(async (command) => {
          if (groupData.status === command.group) {
            try {
              command.function(groupData, { store: store, Void: GiftedConn });
            } catch (error) {
              console.error("[GROUP PARTICIPANTS ADD ERROR] ", error);
            }
          }
          if (/on|true|main|all|gifted|smd/gi.test(command.group)) {
            try {
              command.function(groupData, { store: store, Void: GiftedConn });
            } catch (error) {
  ...
  GiftedConn.ev.on("groups.update", async (groupsUpdate) => {
    try {
  ...
  GiftedConn.ev.on("groups.upsert", async (groupsUpsert) => {
    try {
      events.commands.map(async (command) => {
        if (
          /on|true|main|all|gifted|smd/gi.test(
            command.groupsetting || command.upsertgroup || command.groupupsert
          )
        ) {
          command.function(
            { ...groupsUpsert[0], bot: GiftedConn },
            { store: store, Void: GiftedConn, data: groupsUpsert }
          );
        }
      });
      await groupsg(
  ...
  GiftedConn.ev.on("contacts.upsert", (contactsUpsert) => {
    try {
  ...
  GiftedConn.ev.on("contacts.update", async (contactsUpdate) => {
    for (let contact of contactsUpdate) {
      let decodedJid = GiftedConn.decodeJid(contact.id);
      if (store && store.contacts) {
        store.contacts[decodedJid] = { id: decodedJid, name: contact.notify };
      }
    }
  });
  GiftedConn.serializeM = (message) => smsg(GiftedConn, message, store, false);
  GiftedConn.ev.on("connection.update", async (_0x3fd0b3) => {
    const {
    ...
    if (_0x2224a7 === "connecting") {
      log("Gifted Md Connecting to Whatsapp ⚠️");
    }
    if (_0x2224a7 === "open") {
      if (
        /true|ok|sure|yes/gi.test(global.session_reset) ||
        !GiftedConn.authState.creds?.myAppStateKeyId
      ) {
        log(
        ...
        GiftedConn.ev.session_reset();
      }
      let _0x89b8f6 = GiftedConn.decodeJid(GiftedConn.user.id);
      let _0x11c5d7 = /254728782591|254762016957|17863688449/g.test(
        _0x89b8f6
      );
      let _0x506b6e = false;
      ...
      log("Gifted Md Connected To WhatsApp ✅");
      try {
        try {
          _0x506b6e =
            (await bot_.findOne({
        ...
        if (_0x506b6e && _0x506b6e.plugins) {
          log("Gifted Md Plugins Installed ✅");
          _0xd559f7 = {
            ..._0x506b6e.plugins,
            ..._0xd559f7,
          };
        ...
        if (Object.keys(_0xd559f7 || {}).length > 0) {
            ...
            try {
              ...
              if (_0x28bc47) {
                let _0x224d22 =
                  _0x5701b6 +
                  (_0x4fcba7[_0x5701b6] &&
                  /.js|.smd|.gifted/gi.test(_0x4fcba7[_0x5701b6])
                    ? _0x4fcba7[_0x5701b6]
                    : ".smd");
                const _0x286e03 =
                  plugin_dir +
          ...
          log("\n✅ Gifted Md External Plugins Installed!");
        }
      } catch (_0x42a37a) {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "Cpanel") +
        "\n";
      try {
        ...
        if (_0x4cf26d.total !== 0) {
          _0x1f88ec +=
            "\n𝗡𝗲𝘄 𝗨𝗽𝗱𝗮𝘁𝗲 𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲\nRedeploy Bot/Sync Your Fork as Soon as Possible!\n";
        }
      } catch (_0x16375a) {}
      global.qr_message = {
  ...
  GiftedConn.ev.on("creds.update", creds);
  GiftedConn.lastStatus = async () => {
    console.log("last_status :", last_status);
    return last_status;
  };
  // Decodes a JID (Jabber ID) string
  GiftedConn.decodeJid = (jid) => {
    if (!jid) {
      return jid;
    }
    if (/:\d+@/gi.test(jid)) {
  ...
  GiftedConn.getName = (jid, withoutContact = false) => {
    let decodedJid = GiftedConn.decodeJid(jid);
    let contact;
    ...
    if (decodedJid.endsWith("@g.us")) {
      return new Promise(async (resolve) => {
        ...
        if (!contact.name?.notify && !contact.subject) {
          try {
            contact = (await GiftedConn.groupMetadata(decodedJid)) || {};
          } catch (error) {}
        }
      ...
      contact =
        decodedJid === "0@s.whatsapp.net"
          ...
          : decodedJid === GiftedConn.decodeJid(GiftedConn.user.id)
          ? GiftedConn.user
  ...
  GiftedConn.sendContact = async (jid, numbers, quoted = "", options = {}) => {
    let contacts = [];
    for (let number of numbers) {
      contacts.push({
        displayName: await GiftedConn.getName(number + "@s.whatsapp.net"),
        vcard:
          "BEGIN:VCARD\nVERSION:3.0\nN:" +
          (await GiftedConn.getName(number + "@s.whatsapp.net")) +
          "\nFN:" +
          global.OwnerName +
          "\nitem1.TEL;waid=" +
          number +
  ...
  GiftedConn.messageId = (length = 8, prefix = "GIFTED") => {
  ...
  GiftedConn.send5ButImg = async (
    ...
    let mediaInfo = await prepareWAMessageMedia(
      ...
      {
        upload: GiftedConn.waUploadToServer,
      }
    );
    ...
    GiftedConn.relayMessage(jid, message.message, {
      messageId: GiftedConn.messageId(),
    });
  };

  // Sends a button message with text
  GiftedConn.sendButtonText = (
    jid,
    buttons = [],
    text,
    footer,
    ...
    GiftedConn.sendMessage(jid, buttonMessage, {
      quoted: quoted,
      ...options,
    });
  };
  ...
  GiftedConn.sendText = (jid, text, quoted = "", options) =>
    GiftedConn.sendMessage(
      jid,
      {
        text: text,
        ...options,
  ...
  GiftedConn.sendImage = async (
    ...
    let buffer = Buffer.isBuffer(image)
      ...
      : Buffer.alloc(0);
    return await GiftedConn.sendMessage(
      jid,
      {
        image: buffer,
        caption: caption,
  ...
  GiftedConn.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    GiftedConn.sendMessage(
      jid,
      {
        text: text,
  ...
  GiftedConn.sendImageAsSticker = async (jid, image, options = {}) => {
    let webpImage;
  ...
  GiftedConn.sendVideoAsSticker = async (jid, video, options = {}) => {
    let webpVideo;
    if (options && (options.packname || options.author)) {
  ...
  GiftedConn.sendMedia = async (
    ...
    let fileData = await GiftedConn.getFile(media, true);
  ...
  GiftedConn.downloadAndSaveMediaMessage = async (
    message,
    fileName = "null",
    returnBuffer = false,
    createFile = true
    ...
    try {
      ...
      if (type == "conversation") {
        ...
        for (let jid of parsedJid(jids)) {
          await GiftedConn.sendMessage(jid, forwardOptions, {
            quoted: quoted,
            messageId: GiftedConn.messageId(),
          });
        }
  ...
  GiftedConn.downloadMediaMessage = async (message) => {
    let msg = message.msg ? message.msg : message;
    let mimeType = (message.msg || message).mimetype || "";
    let mediaType = message.mtype
      ? message.mtype.replace(/Message/gi, "")
  ...
  GiftedConn.forwardOrBroadCast2 = async (
    jid,
    message,
    options = {},
    type = ""
    ...
    try {
      ...
      await GiftedConn.relayMessage(jid, forwardedMessage.message, {
        messageId: GiftedConn.messageId(),
      });
  ...
  GiftedConn.forwardOrBroadCast2 = async (
    ...
    try {
      ...
      await GiftedConn.relayMessage(_0x2565bd, _0x5d8b14.message, {
        messageId: GiftedConn.messageId(),
      });
      return _0x5d8b14;
    } catch {}
  };
  GiftedConn.forwardOrBroadCast = async (
    _0x189e5c,
    _0x54c0d9,
    _0x18e0cf = {},
    _0x348d78 = ""
    ...
    try {
      ...
      _0x18e0cf.messageId = _0x18e0cf.messageId || GiftedConn.messageId();
      ...
      await GiftedConn.relayMessage(_0x189e5c, _0xf31bf3.message, {
  ...
  GiftedConn.forwardMessage = GiftedConn.forwardOrBroadCast;
  GiftedConn.copyNForward = async (
    _0x333b21,
    _0x4d04c0,
    _0x3c23fa = false,
    _0x4ed2c0 = {}
    ...
    try {
      ...
      await GiftedConn.relayMessage(_0x333b21, _0x3f7fe3.message, {
        messageId: GiftedConn.messageId(),
      });
      return _0x3f7fe3;
    } catch (_0x529a5c) {
  ...
  GiftedConn.sendFileUrl = async (
    ...
    try {
      ...
      if (_0x5a6000) {
        try {
          return await GiftedConn.sendMessage(_0x245d2a, _0x5a6000, {
            quoted: _0x2cf1f3,
          });
        } catch {}
      }
  ...
  GiftedConn.sendFromUrl = GiftedConn.sendFileUrl;
  const _0x29f5c5 = {};
  let _0x51034c = [];
  GiftedConn.sendUi = async (
    _0x264148,
    _0x4d42ab = {},
    _0x541cb3 = "",
    _0x2e5e1f = "",
    ...
    try {
      ...
      if (
        (((_0x171bcf && _0x3bceba && global.style > 0) || !_0x3bceba) &&
          /text|txt|nothing|smd|gifted/.test(global.userImages)) ||
        _0x2e5e1f == "text"
      ) {
        _0x237b02 = {
    ...
    try {
      return await GiftedConn.sendMessage(_0x264148, {
        image: {
          url: await botpic(),
        },
        contextInfo: _0x28cdb7,
      ...
      return GiftedConn.sendMessage(_0x264148, {
        text: _0x4d42ab.text || _0x4d42ab.caption,
        ..._0x4d42ab,
      });
    }
  };
  GiftedConn.contextInfo = async (
  ...
  GiftedConn.cMod = (
    ...
    _0x4510a5.key.fromMe = _0x2f32eb === GiftedConn.user.id;
    return proto.WebMessageInfo.fromObject(_0x4510a5);
  };
  GiftedConn.getFile = async (_0x45942b, _0x80d77a) => {
    let _0x5bc7b0;
  ...
  GiftedConn.sendFile = async (
    ...
    let _0x4285a2 = await GiftedConn.getFile(_0x2edb21, true);
    let {
      filename: _0x479138,
      size: _0x527df9,
      ext: _0x14fc23,
  ...
  GiftedConn.parseMention = async (_0x3d4032) => {
  ...
  app.get("/chat", (_0x52c0af, _0x203368) => {
    ...
    _0x5785a4 = GiftedConn.decodeJid(_0x5785a4);
    const _0x382b66 =
      (
  ...
  GiftedConn.dl_size = global.dl_size || 200;
  GiftedConn.awaitForMessage = async (_0x3f601c = {}) => {
    return new Promise((_0x42ef72, _0x15fbb5) => {
      ...
      let _0x4fedac = (_0x54d8e0) => {
        ...
        if (_0x4bad66 == "notify") {
          for (let _0xa0c0c6 of _0x5bf6f0) {
            ...
            const _0x5447cd = GiftedConn.decodeJid(
              _0x3f5f64
                ? GiftedConn.user.id
                : _0x36ea4b || _0x27529b
            ...
            if (
              ...
              GiftedConn.ev.off("messages.upsert", _0x4fedac);
      ...
      GiftedConn.ev.on("messages.upsert", _0x4fedac);
      if (_0x358393) {
  ...
  try {
    fs.readdirSync(pluginsDir).forEach((file) => {
      ...
      if (fs.statSync(filePath).isDirectory()) {
        ...
        [".js", ".smd", ".gifted"].includes(path.extname(file).toLowerCase())
      ) {
        try {
          require(filePath);
        } catch (error) {
...
<body>
    <link rel="icon" href="https://telegra.ph/file/54efddccf41281ad7ec51.jpg">
    <div class="status">GIFTED-MD IS ACTIVE</div>
</body>
</html>
`;
...
app.get("/gifted", (request, response) => response.type("html").send(html));
...
app.listen(randomPort, () =>
  console.log(
    "Gifted-Md Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  let decodedSessionId = ("" + sessionId)
    ...
    .replace(/^GIFTED_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "")
    .replace(/Secktor;;;/gi, "")
    .replace(/Giftech;;;/gi, "")
    .replace(/Gifted;;;/gi, "")
    .trim();
  ...
  const officialRepoPath = "/giftedtechnexus/";
  const isOfficialRepo =
    toBool(isOfficial || global.IS_GIFTEDTECH || process.env.IS_GIFTEDTECH, true) ||
  ...
  if (isOfficialRepo) {
    ...
    GiftedOfficial = false;
    log(
      "\n\nYou are using a Modified/Edited Version. Please Deploy Gifted-Md Bot from the Original Repository.\nDeploy From : https://github.com" +
        officialRepoPath +
        "Gifted-Md\n"
    );
    process.exit(0);
