...
const giftech_patch = require("../lib/plugins");
...
➮Nᴜᴍ - ${owner.split(",")[0]} 
➮Mᴇᴍ - ${formatp(os.totalmem() - os.freemem())} 
      \n *ɢɪғᴛᴇᴅ ᴍᴅ ᴡʜᴀᴛsᴀᴘᴘ ʙᴏᴛ*\n\n ${readmore} 
╭──❰ *ALL MENU* ❱ 
│🎗 Lɪꜱᴛ 
│🎗 Cᴀᴛᴇɢᴏʀʏ 
│🎗 Hᴇʟᴘ 
...
giftech_patch.cmd(
  ...
  async (message, query, { Void }) => {
    try {
      ...
      const foundCommand =
        giftech_patch.commands.find((cmd) => cmd.pattern === originalCommand) ||
        giftech_patch.commands.find(
...
giftech_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*Gifted Pinging!*");
...
giftech_patch.cmd(
  ...
  async (message) => {
    try {
      message.reply(
        `*Uptime of ${tlang().title}: ${runtime(process.uptime())}*`
      );
    } catch (error) {
      ...
      let contactMessage = {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            thumbnail: log0,
            mediaType: 1,
            mediaUrl: "https://telegra.ph/file/54efddccf41281ad7ec51.jpg",
